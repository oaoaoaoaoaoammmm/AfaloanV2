...
class AuthenticationProvider(
    ...
    @PostConstruct
    fun init() {
        this.key = Keys.hmacShaKeyFor(authenticationProperties.secret.toByteArray())
    }
    ...
    fun createAccessToken(userId: UUID, username: String, roles: Set<Role>): String {
        val claims = Jwts.claims()
            ...
            .add("roles", roles)
            .build()
        val validity = Instant.now().plus(authenticationProperties.access, ChronoUnit.HOURS)
        return Jwts.builder()
            .claims(claims)
    ...
    fun createRefreshToken(userId: UUID, username: String): String {
        val claims = Jwts.claims()
            ...
            .add("id", userId)
            .build()
        val validity = Instant.now().plus(authenticationProperties.refresh, ChronoUnit.HOURS)
        return Jwts.builder()
            .claims(claims)
            .expiration(Date.from(validity))
            .signWith(key)
